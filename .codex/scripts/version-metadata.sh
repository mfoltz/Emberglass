...
CSPROJ_PATH="$REPO_ROOT/Emberglass.csproj"
...
read_manifest_version() {
    local version
    version=$(sed -n 's/^  "version_number": "\([^"]*\)",$/\1/p' "$MANIFEST_PATH" | head -n 1 | tr -d '[:space:]')

    if [ -z "$version" ]; then
        fail "Unable to determine manifest version from $MANIFEST_PATH."
    fi
...
read_latest_changelog_entry() {
    local entry
    entry=$(sed -n 's/^## v\([0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\)$/\1/p' "$CHANGELOG_PATH" | head -n 1 | tr -d '[:space:]')
...
require_matching_version "$MANIFEST_PATH" "$(read_manifest_version)" "$canonical_version"
