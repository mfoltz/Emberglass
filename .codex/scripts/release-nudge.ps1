...
function Test-MeaningfulPath {
    ...
    return $NormalizedPath -match '^(API|Config|Network|Patches|Services|Systems)/' `
        -or $NormalizedPath -match '^\.codex/scripts/' `
        -or $NormalizedPath -match '^\.github/workflows/' `
        -or $NormalizedPath -eq 'Emberglass.csproj'
}
...
if ($MeaningfulFiles.Count -eq 0) {
    Write-Host "release-nudge: no meaningful source, API, script, or workflow changes detected."
    exit 0
}
...
$ApiTouched = @($MeaningfulFiles | Where-Object { ($_ -replace '\\', '/') -match '^(API|Systems)/' }).Count -gt 0
...
if (($ApiTouched -or $ChangedLines -ge $LineThreshold) -and -not $ChangelogChanged -and -not $HasUnreleasedNotes) {
    Write-Nudge "Meaningful Emberglass changes detected ($ChangedLines changed lines across $($MeaningfulFiles.Count) files). Consider adding CHANGELOG.md notes before release."
}

if ($HasUnreleasedNotes) {
    Write-Nudge "CHANGELOG.md has Unreleased notes. Before a release-bound merge, consider running .codex/scripts/bump-version.ps1 so version metadata and changelog stay aligned."
}
